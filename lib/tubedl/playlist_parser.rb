module PlaylistParser
  ...
  class Playlist
    attr_reader :playlistID, :playlist
    ...
    def initialize(link = nil)
      ...
      playlist_data.each_pair do |video_name, video_link|
        ...
        instance_variable_set("@#{counter.succ!}", video_name => video_link)
    ...
    def parse_page(page)
      ...
      if playlist_data
        songs_hashes = Hash[playlist_data
                            .css('a.pl-video-title-link').map do |ln|
                              [ln.text.strip, "https://www.youtube.com#{ln['href']}"
    ...
    def save_playlist(song_hash)
      File.write("Playlist-#{Time.now.to_a[3..5].join}.yml", song_hash.to_yaml)
    end
  end
end
